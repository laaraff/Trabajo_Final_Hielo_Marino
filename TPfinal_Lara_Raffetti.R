...
promedio_soi_diciembre <- function(estacion) { 
  #voy a usar esa funcion solo para diciembre, por lo que espero que lo unico ingresado sea "DEF"
  promedios <- data.frame()
  if (estacion=="DEF") {
...
ggplot() +
  ...
  theme_bw() + 
  theme(
    panel.background = element_rect(fill = "#acd8fa") 
  ) 
